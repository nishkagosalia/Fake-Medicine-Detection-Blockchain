...
const RetailerPlaceOrder = () =>{
   ...
   useEffect(() => {
    ...
    const getMeds = async() =>{
      ...
      const medicinearray=await fetch('http://192.168.45.183:3000/medslist',{method:'GET'})
     ...
     const getAllMedicineData = async() =>{
      ...
      const allMedData = await fetch('http://192.168.45.183:3000/getAllMedsDB',{method:'GET'});
     ...
     const doMultipleThings = (itemValue) =>{
      ...
      const placeMedsOrder = async() =>{
        ...
        await fetch('http://10.120.110.69:3000/placeMedsOrder',{
            method:'POST',
            headers:{
